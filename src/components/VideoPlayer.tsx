...
export default function VideoPlayer({
  ...
  const [isCrosshairZoomed, setIsCrosshairZoomed] = useState(false);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 'z') {
        setIsCrosshairZoomed((prev) => !prev);
      }
    };
    ...
    >
        ...
        <video
          ...
          style={{
            ...
            transform: isCrosshairZoomed ? "scale(2.3)" : "none",
            transformOrigin: "center 42%",
          }}
          className="w-full h-full object-cover cursor-pointer transition-transform duration-300 pointer-events-auto select-none"
        />

        {/* Crosshair Zoom Center Reticle Overlay */}
        {isCrosshairZoomed && (
          <div className="absolute inset-0 pointer-events-none z-20 flex items-center justify-center">
            <div className="relative w-12 h-12 flex items-center justify-center -translate-y-[8%]">
              <div className="absolute w-8 h-8 rounded-full border border-dashed border-[var(--color-val-red)]/80 animate-spin"></div>
              <div className="w-1.5 h-1.5 rounded-full bg-[var(--color-val-red)] shadow-[0_0_8px_#ff4655]"></div>
              <div className="absolute -top-3 text-[9px] font-black uppercase text-[var(--color-val-red)] tracking-wider bg-black/75 px-1.5 py-0.2 rounded-full border border-[var(--color-val-red)]/40">
                Zoom 2.3x
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div className="flex items-center justify-between text-white text-[11px] sm:text-xs font-bold pt-0.5 sm:pt-1">
            ...
            {/* Right: Zoom Réticule, Vitesse, Qualité, Fullscreen */}
            <div className="flex items-center gap-1.5 sm:gap-2.5 relative">
              {/* Zoom Réticule Magnifier Button */}
              <button
                onClick={() => setIsCrosshairZoomed(!isCrosshairZoomed)}
                title="Zoom Réticule (Raccourci: Z)"
                className={`px-1.5 sm:px-2 py-0.5 rounded transition-all text-[9px] sm:text-[10px] uppercase font-bold flex items-center gap-1 border cursor-pointer ${
                  isCrosshairZoomed
                    ? "bg-[var(--color-val-red)] border-[var(--color-val-red)] text-white shadow-[0_0_10px_rgba(255,70,85,0.5)]"
                    ...
                    {speeds.map((s) => (
                      <button
                        ...
                        className={`w-full text-left px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] hover:bg-[var(--color-val-red)]/20 hover:text-[var(--color-val-red)] transition-colors flex items-center justify-between cursor-pointer ${
                          playbackRate === s ? 'text-[var(--color-val-red)] font-black' : 'text-white/80'
                        }`}
                      >
                        <span>{s}x</span>
                        {playbackRate === s && <span>✓</span>}
                      </button>
                    ))}
                  </div>
                )}
