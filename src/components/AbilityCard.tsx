          ...
          <div style={{ display: activeTab === 'video' ? 'block' : 'none' }} className="animate-in fade-in duration-300">
            <div className="relative w-full aspect-video bg-black rounded-lg overflow-hidden border border-white/10 group">
              ...
              {isDescriptionExpanded && currentVideo?.description && (
                <div className="absolute inset-0 bg-black/92 backdrop-blur-md z-50 flex flex-col p-2.5 xs:p-3 sm:p-5 md:p-6 animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex justify-between items-center mb-1.5 sm:mb-3 pb-1 sm:pb-1.5 border-b border-white/10 flex-shrink-0">
                    <h4 className="text-white font-bold uppercase tracking-widest text-[10px] xs:text-xs sm:text-sm flex items-center gap-1.5 truncate">
                      <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-[#fa4454] flex-shrink-0"></span>
                      <span className="truncate">{tr("Détails :")} {currentVideo.label}</span>
                    </h4>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="text-white/60 hover:text-white transition-colors p-0.5 sm:p-1 cursor-pointer flex-shrink-0"
                    >
                  ...
                  <div className="flex-1 overflow-y-auto text-white/90 text-xs sm:text-sm md:text-base leading-snug sm:leading-relaxed pr-1 sm:pr-2 custom-scrollbar">
                    <RichTextRenderer content={currentVideo.description} />
                  </div>
                  <div className="pt-1.5 sm:pt-3 mt-auto text-center flex-shrink-0">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setIsDescriptionExpanded(false);
                      }}
                      className="inline-flex items-center gap-1 text-[10px] sm:text-xs font-bold uppercase tracking-widest text-white/90 hover:text-white bg-white/10 hover:bg-[#fa4454] px-4 sm:px-6 py-1 sm:py-1.5 rounded-full transition-colors border border-white/10 cursor-pointer shadow-sm"
                    >
                      <span>{tr("Fermer")}</span>
